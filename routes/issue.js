...
exports.plugin = function(app, environment, ppt, isPrivatePortal) {
	...
	environment.addApplicationToMenu("/issue","Issue");
	  ...
	  // Routes
	  /////////////////
	app.get('/issue', isPrivate, function issueGet(req, res) {
	   ...
	   */
	app.get('/issue/new', isLoggedIn, function issueGetNew(req, res) {
		var data =  myEnvironment.getCoreUIData(req);
	   ...
	   */
	app.get("/issue/index", isPrivate, function issueGetIndex(req, res) {
		var start = parseInt(req.query.start);
		var count = parseInt(req.query.count);
		var credentials= [];
		if (req.user) {credentials = req.user.credentials;}

		IssueModel.fillDatatable(start,count, credentials, function issueFillTable(data, countsent, totalavailable) {
	...
	var _issuesupport = function(body,usx, callback) {
		...
		if (body.locator === "") {
			IssueModel.create(body, usx, credentials, function issueCreate(err, result) {
				return callback(err,result);
			});
		} else {
	        IssueModel.update(body, usx, credentials, function issueUpdate(err, result) {
	            return callback(err,result);
	        });
		}
	};
	   ...
	   */
	app.post('/issue', isLoggedIn, function issuePost(req, res) {
	    var body = req.body;
	    var usx = req.user;
	    myEnvironment.logDebug('ISSUE_NEW_POST '+JSON.stringify(usx)+' | '+JSON.stringify(body));
	    _issuesupport(body, usx, function issueSupport(err, result) {
	...
	app.get("/issue/ajaxfetch/:id", isPrivate, function issueGetAjax(req, res) {
			...
			Dataprovider.getNodeByLocator(q, credentials, function issueGetNode(err, result) {
				...
				if (result) {
					...
					CommonModel.__doAjaxFetch(result, credentials, "/issue/", tags, docs, users, transcludes, data, req, function issueDoAjax(json) {
						myEnvironment.logDebug("Issue.ajaxfetch-1 "+JSON.stringify(json));
							//send the response
							return res.json(json);
					});
				} else {
					return res.redirect('/error/IssueCannotDisplay');
				}
			});
	});
		   ...
		   */
	app.get('/issue/:id', isPrivate, function issueGetId(req, res) {
			var q = req.params.id;
			var data = myEnvironment.getCoreUIData(req);
			myEnvironment.logDebug("ISSUY "+JSON.stringify(req.query));
			CommonModel.__doGet(q,"/issue/",data, req, function issueDoGet(viewspec, data) {
